...
"""Legacy Megabyte statistical compression diagnostics on selected DNA splits.
...
scripts/run_dna_compression.py for actual Window Codec compression.

DNACorpus legacy statistical compression with the OpenGenome2-finetuned
checkpoint, matching the existing GeCo2 paper-mode baseline source order and
0.6/0.2/0.2 split:

    CUDA_VISIBLE_DEVICES=2 python scripts/run_dna_compression_legacy.py \
...
No-split legacy statistical compression over each full source:

    CUDA_VISIBLE_DEVICES=1 python scripts/run_dna_compression_legacy.py \
      --run-dir outputs/dna_megabyte_large_20260616_144744_20260616_171309_20260617_140217 \
      --checkpoint-tag best \
      --output-json outputs/dna_megabyte_large_20260616_144744_20260616_171309_20260617_140217/statistics_dnacorpus_full/compression_compare.json \
      --export-out-dir outputs/dna_megabyte_large_20260616_144744_20260616_171309_20260617_140217/statistics_dnacorpus_full \
      --dataset-dir datasets/DNACorpus \
      --sequence-source-mode auto \
      --multi-sequence-mode separate \
      --split full \
...
def _apply_overrides(config: ExperimentConfig, args: argparse.Namespace) -> None:
    ...
    _apply_if_not_none(config, "arithmetic.quantization_mode", args.arithmetic_quantization_mode)
...
def _validate_args(config: ExperimentConfig, args: argparse.Namespace) -> None:
    ...
    if config.arithmetic.quantization_mode not in MEGABYTE_ARITHMETIC_QUANTIZATION_MODES:
        raise ValueError(
            "arithmetic.quantization_mode must be one of: "
            + ", ".join(MEGABYTE_ARITHMETIC_QUANTIZATION_MODES)
        )
    if (
        config.arithmetic.quantization_mode == "fast_floor_gpu"
        and any(mode != NON_OVERLAP_MODE for mode in args.compression_modes)
    ):
        raise ValueError("fast_floor_gpu is only implemented for --compression-modes windows_nonoverlap.")
...
def _write_window_codec_payload(
    *,
    ...
    target_dir = out_dir / _sanitize_artifact_name(split_name) / _sanitize_artifact_name(mode_name)
...
def _run_split(
    *,
    ...
    include_codec_baselines: bool,
    window_codec_out_dir: Path | None,
    window_codec_batch_size: int,
) -> dict[str, object]:
    ...
    for mode in modes:
        ...
        for source_index, (entry, source) in enumerate(zip(source_entries, sources), start=1):
            ...
            if window_codec_out_dir is not None and mode == NON_OVERLAP_MODE:
                print(
...
def main() -> None:
    ...
    if window_codec_out_dir is not None:
        raise ValueError(
            "scripts/run_dna_compression_legacy.py is the old statistical/non-.mbw path and does "
            "not support --window-codec-out-dir. Use scripts/run_dna_compression.py for actual "
            "Window Codec compression."
        )

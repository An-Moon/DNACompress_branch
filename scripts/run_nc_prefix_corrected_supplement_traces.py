...
"""Run nc_prefix target-probability traces for the corrected annotation supplement.

Example:
    python scripts/run_nc_prefix_corrected_supplement_traces.py \
      --dataset-root datasets/dnacorpus_corrected_supplement_v1 \
      --output-dir outputs/nc_prefix_dnacorpus_corrected_supplement_w8192_target_traces \
      --position-major-output-dir outputs/nc_prefix_dnacorpus_corrected_supplement_w8192_target_traces_position_major \
      --window-bases 8192 \
      --nc-prefix-preset geco2_paper \
      --nc-prefix-backend fast_cpp
"""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate nc_prefix target-probability traces for dnacorpus_corrected_supplement_v1."
    )
    parser.add_argument("--dataset-root", default="datasets/dnacorpus_corrected_supplement_v1")
    parser.add_argument(
        "--output-dir",
        default="outputs/nc_prefix_dnacorpus_corrected_supplement_w8192_target_traces",
        help="Depth-major trace root.",
    )
    parser.add_argument(
        "--position-major-output-dir",
        help="Position-major trace root. Defaults to <output-dir>_position_major.",
    )
    ...
    parser.add_argument(
        ...
        default="geco2_paper",
        help="fixed uses --nc-prefix-geco2-level for all sources; geco2_paper uses the DNACorpus paper map.",
    )
...
def _trace_command(args: argparse.Namespace, source_row: dict[str, Any], trace_dir: Path, geco2_level: int) -> list[str]:
    return [
        sys.executable,
        ...
        rows.append(
            {
                ...
                or math.ceil(max(row_count, 1) / max(int(args.window_bases), 1)),
                "token_merge_size": manifest.get("token_merge_size"),
                "geco2_level": producer.get("nc_prefix", {}).get("geco2_level")
                if isinstance(producer.get("nc_prefix"), dict)
                else _resolve_geco2_level(source, str(args.nc_prefix_preset), int(args.nc_prefix_geco2_level)),
    ...
    for source_row in source_rows:
        ...
        with log_path.open("w", encoding="utf-8") as log:
            if not (args.skip_existing and depth_complete):
                cmd = _trace_command(args, source_row, trace_dir, geco2_level)
        ...
        _summarize(output_dir, position_major_output_dir, source_rows, args=args)
